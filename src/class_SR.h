...
public:
    ...
    void mine(const char *X, uint8_t num_bytes_X,int Threshold){    
        cst_t cst;
...
void HRSF (const char* X, uint8_t num_bytes_X, int Threshold){
     //Highly Repeated Substring Finder
     
    return (example.mine(X,num_bytes_X,Threshold));
}
