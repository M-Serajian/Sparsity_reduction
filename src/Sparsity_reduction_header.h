...
void find_repetitive_sequences (const char *X, uint8_t num_bytes_X,int k, int l){    
    cst_t cst;
    construct_im(cst, X, 1);
    node_t root= cst.root();
    int nodes= cst.nodes();
...
int counter=0;
//checking the length of the substrings
for (int i=0;i<nodes;i++){
    if(flag[i]==1){
        int d=cst.depth(NodeCST[i]);
        if (d >= l){
            counter++;
        }else {
            flag[i]=0;
...
for (int i=0;i<nodes;i++){
    if(flag[i]==1){
    cout<< "For the node with id=  "<< i <<", the substring which is repeated at least " << k <<" times is : " << extract(cst, NodeCST[i]) <<endl;
    }
}
return;
}
...
string input_file_reader (string address)
{
    ifstream Input_file;
    ...
    while( getline( Input_file, line ).good() ){
        if( line.empty() || line[0] == '>' ){ // Identifier marker- For new Lines
    ...
    out.erase(out.size()-1,1);// removing the last charater of the string ($) since 
    // CST ads one to the end of string on its own. we do not want to have $$ at the of sequence
    // end!
    return (out);
}
